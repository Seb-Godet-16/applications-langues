/* ============================================================
   ...
   et Taphad'Meuh (français-oromo) :
     ...
     07/06/2026 → 29/06/2026  Versions Bêta créées avec IA Claude
                               Sonnet 4.6 et Gemini 3.5 Flash.

     29/06/2026 → 08/07/2026  Recettages et correctifs avec IA
                               Claude Sonnet 5 et Gemini 3.5 Flash.

     08/07/2026 → 12/07/2026  Retours d'expériences utilisateurs
                               (Christophe, Maman, Moi) avec des
                               correctifs réalisés avec IA Claude
                               Sonnet 5.

     18/07/2026 → 25/07/2026  Retours d'expérience utilisatrice
                               Sandrine avec des correctifs réalisés
                               avec IA Claude Sonnet 5.

     27/08/2026 → 31/08/2026  Retours d'expérience utilisateur Mussa
                               et Moi avec des correctifs réalisés
                               avec IA Claude Sonnet 5.

     05/09/2026               Mise à jour Qui suis-je + Historique
                               avec IA Claude Sonnet 5.
   ============================================================
   ARCHITECTURE (5 fichiers) :
     ...
     └─ app.js      → Ce fichier : logique applicative complète (6 326 lignes le 05/09/2026 — chiffre à revérifier périodiquement, cf. note du PLAN DU FICHIER ci-dessous)

   PLAN DU FICHIER (décalage mécanique appliqué le 05/09/2026, suite à
   la condensation de l'historique ci-dessus, demande utilisateur
   commune à VACHÉBO et Taphad'Meuh — seul HISTORIQUE DE L'APPLICATION
   a été raccourci, aucune ligne de code fonctionnel modifiée. Ce bloc
   précédant l'intégralité du fichier, le décalage résultant est
   uniforme sur toutes les ancres ci-dessous ; il a été calculé
   précisément (nouveau total de lignes moins ancien), puis chaque
   ancre a été retranslatée d'autant et un échantillon (§0, §7, §17,
   §21c) spot-vérifié par grep contre le fichier réellement réécrit,
   plutôt que recalculé un par un comme lors des précédentes passes) :
     §0    L.  204  Chargement conditionnel des données — loadDataForMode() (L.355)
     §0b   L.  233  Helpers globaux — showResetConfirm(), confirmReset(), cancelReset(),
                     spinner (_showSpinner()/_hideSpinner()), _launchConfetti() (L.280) ;
                     placés avant loadDataForMode() dans le fichier pour raison de
                     hoisting (cf. note d'ordre juste au-dessus de showResetConfirm())
     §1    L.  391  Variables d'état globales
     §1b   L.  449  Utilitaires bilingues — L(), isFrench(), langKeys(), _themeTitle()
     §3    L.  532  Point d'entrée — showLauncherVariant() (L.551), initApp() (L.735),
                     showLauncher() (L.701)
     §3b   L.  877  Synthèse vocale — _getTtsSpeed()/_setTtsSpeed()/_getTtsRepeat()/
                     _toggleTtsRepeat(), _resolveSpanishVoice() (L.950), speak() (L.1175),
                     speakSlow() (L.1357), _updateVoiceBadge() (L.1373).
                     _stopSpeaking() (L.1337, ajoutée le 28/08/2026, absente
                     ...
                     _toggleLessonSettings() (L.1420), _updateSpeedBar() (L.1432),
                     _buildSpeedBar() (L.1458), _tabHasPdf() (L.1495, ajoutées le
                     ...
                     2 rangées fixes.
     §3a-bis L. 1094  Surlignage mot par mot pendant la lecture (TTS, best-effort)
     §3c   L. 1542  Interruption TTS à la mise en arrière-plan (visibilitychange)
     §3d   L. 1563  Keepalive watchdog Chrome/Android (pause/resume toutes les 8 s)
     §3e   L. 1593  Audio indisponible + toast _showToast() (L.1642) + _vibrateFeedback() (L.1668)
     §4    L. 1677  Persistance — loadDone() (L.1693), suivi modules ouverts (12/07), étoiles, quiz
     §5    L. 1894  Navigation — showScreen() (L.2159), _showScreenNoRender() (L.2125),
                     _updateBottomNav() (L.1916)
     §5b   L. 2022  Helpers niveaux — _updateLevelTabs() (L.2046), lessonGoBack() (L.2068),
                     navGoModules() (L.2082)
     §6    L. 2249  Écran Home — renderHome() (L.2275), _renderHomeRegionWidget() (L.2393)
     §7    L. 2439  Écran Sections — renderSections() (L.2602), _buildThemeCard() (états + badge, 12/07).
                     ...
                     à l'intérieur d'une catégorie). Toujours le 27/08/2026
                     (suite, même demande étendue au Niveau 2, "pour aider
                     l'apprenant") : LEVEL2_CATEGORIES (4 groupes, 16
                     dialogues) applique le même principe au Niveau 2 ;
                     _buildThemeGridHTML(level) choisit désormais le bon
                     tableau de catégories selon le niveau au lieu de
                     retomber sur une grille à plat pour le Niveau 2.
                     _buildContinueCard() affiche par ailleurs une carte
                     "▶ Continuer" en tête de grille (Niveau 1 et 2)
                     pointant vers le prochain thème pertinent.
                     renderSections() délègue le remplissage de grid1/grid2
                     à _buildThemeGridHTML() (au lieu d'un .map() direct sur
                     _buildThemeCard()).
                     Appelle aussi _markModulesVisited() (§15d, appel L.2611)
     ...
     §8    L. 2780  Ouverture d'un thème — openTheme() (L.2792, marque module ouvert), switchTab()
     §9    L. 3024  Cartes Flash — renderFlash() (L.3040), pickAlpha(), buildAlphaDetail().
     ...
     §9b   L. 3271  Reconnaissance vocale — _normalizeSpeech(), _levenshtein(), _speechMatch()
     §9c   L. 3688  Onglet Répète — renderRepeat(), _rpShowWord(), _rpStartMic(), _rpShowEnd().
                     Clin d'œil mascotte sur sans-faute ajouté à _rpShowEnd() le
                     20/07/2026 (demande utilisateur), via _mascotSansFauteLine() (§14).
     §10   L. 4049  Quiz 10 questions — _generateLevel1Quiz(), renderQuiz10(), checkQ10()
     §11   L. 4327  Dialogue — _adaptDialogueLine(), renderDialog(), pickSit()
     §12   L. 4448  Vocabulaire — renderVocab() (chips cliquables)
     §13   L. 4506  Quiz Dialogue — renderDialogQuiz(), checkDQ()
     §14   L. 4600  Utilitaires — _quizResultStrings(), _mascotSansFauteLine() (L.4655),
     ...
     §15   L. 4685  Variantes régionales — renderRegionGrid(), pickRegion(), changeRegion().
     ...
     §15b  L. 5035  Accordéons — toggleAcc() (L.5046), toggleLevelAcc(), _resizeOpenAccordions()
     ...
     §15c  L. 5100  Nouvel utilisateur & barre de nav — _isBrandNewUser(), condition
                     ...
                     showLauncherVariant())
     §15d  L. 5131  Cartes de langue fusionnées avec l'explicatif — _langBoxInitialOpen(),
     ...
     §18   L. 5836  E-mail antispam — openAndCopyEmail() (L.5850)
     §19   L. 5869  Exports PDF — _pdfTheme() (L.5883), _exportGuide(), _exportVocab(), _exportSituation()
     ...
     §20   L. 6182  Accessibilité clavier (keydown → role="button")
     §21   L. 6197  Initialisation Launcher — addEventListener sur les cartes de langue
     §21b  L. 6223  Viewport height fix Android — --app-h via window.innerHeight
     §21c  L. 6264  Bouton d'installation PWA native — _initInstallButtons() (L.6294),
                     _installPwa() (L.6310) ; bouton #homeInstallBtn (tête de l'écran
 ...
 *
 * Note : les deux parcours partagent toujours le même total de 53 modules
 * (37 thèmes + 16 dialogues depuis le 26/08/2026, cf. Bilan_technique.md) — on réutilise donc
...
function _renderOtherModeProgress() {
  ...
  const total       = ALL_THEMES.length || 53;
...
/* ═══════════════════════════════════════════════════════════
   ...
   Regroupement par catégories + carte "Continuer" — ajouté le
   26/08/2026 (suite, demande utilisateur : la grille à plat des 37
   thèmes du Niveau 1 pouvait faire l'effet d'un "mur de cartes"
   décourageant au premier coup d'œil pour un grand débutant).
   THEME_CATEGORIES (5 groupes, 37 thèmes) puis LEVEL2_CATEGORIES
   (4 groupes, 16 dialogues — ajouté à son tour le 27/08/2026, même
   demande utilisateur étendue au Niveau 2 "pour aider l'apprenant")
   regroupent les cartes des 2 niveaux sous des <details>/<summary>
   natifs — repli/dépli sans JS de toggle dédié, accessible clavier +
   lecteur d'écran par défaut du navigateur. Seule la catégorie "en
   cours" (1ère catégorie contenant un thème pas encore à 3 étoiles)
   est dépliée par défaut, le reste replié — même logique pour les 2
   niveaux dans _buildThemeGridHTML(), qui choisit simplement le bon
   tableau de catégories selon le niveau demandé.
   _buildContinueCard() ajoute par ailleurs, au-dessus de la grille
   des 2 niveaux, un bouton "▶ Continuer" pointant directement vers
   le prochain thème pertinent (en cours, sinon jamais ouvert), pour
   ne pas obliger à parcourir la grille pour reprendre sa leçon.
   Affichage pleine largeur — ajusté le 27/08/2026 (demande
   utilisateur, capture d'écran à l'appui) : #grid1/#grid2 (les
   catégories des 2 niveaux) réutilisaient la classe .themes-grid en
   grille 2 colonnes, pensée pour des cartes de thème individuelles —
   appliquée par erreur aux <details> de catégorie eux-mêmes, elle
   les empilait 2 par ligne et les rétrécissait de moitié (cartes
   tronquées à l'écran). #grid1/#grid2 passent en colonne unique dans
   style.css ; seule .theme-subgrid (cartes à l'intérieur d'une
   catégorie) garde ses 2 colonnes. Et comme deux catégories dépliées
   côte à côte sur 2 colonnes trop étroites devient une seule
   catégorie dépliée en pleine largeur, n'ouvrir que la catégorie "en
   cours" (et non plus courante + suivante, voir ci-dessus) évite un
   écran d'un coup trop chargé.
═══════════════════════════════════════════════════════════ */

/* THEME_CATEGORIES — 5 regroupements thématiques du Niveau 1 (37
   thèmes au total, tous couverts) ; LEVEL2_CATEGORIES (juste après)
   fait de même pour les 16 dialogues du Niveau 2, en 4 groupes.
   Purement un regroupement d'affichage (accordéon) dans les 2 cas :
   ne modifie ni l'ordre de progression, ni les données de
   data-fr.js/data-es.js, ni les clés de sauvegarde
   (`done`/`openedThemes`) — indexé uniquement par thème.id, résolu
   dynamiquement contre ALL_THEMES à chaque rendu. */
const THEME_CATEGORIES = [
  { id: 'bases',     emoji: '🌱', labelEs: 'Lo Esencial',           labelFr: 'Les Bases',
    themeIds: ['alpha','salut','pres','express','interrog','num','verb','conn'] },
  { id: 'describir', emoji: '😊', labelEs: 'Describir y Sentir',    labelFr: 'Décrire & Ressentir',
    themeIds: ['heure_calendrier','meteo_climat','adj','emot','fam','col','corps'] },
  { id: 'quotidien', emoji: '🏡', labelEs: 'Vida Cotidiana',        labelFr: 'Vie Quotidienne',
    themeIds: ['adv','viequot','nour','bois','objets','log','muebles_equipamiento','veth'] },
  { id: 'sorties',   emoji: '🚌', labelEs: 'Salidas y Situaciones', labelFr: 'Sorties & Situations',
    themeIds: ['rest','orient','lieux','trans','met','sante','securite','dinero'] },
  { id: 'monde',     emoji: '🌍', labelEs: 'El Mundo',              labelFr: 'Le Monde',
    themeIds: ['viaje','pays','ocio','plan','anim','agric'] }
];

/* Niveau 2 (16 dialogues) — ajouté le 27/08/2026 (demande utilisateur :
   "pour aider l'apprenant", suite au regroupement du Niveau 1 la veille).
   Même principe de catégories repliables que THEME_CATEGORIES ci-dessus,
   avec un découpage propre aux dialogues plutôt qu'au vocabulaire :
   4 groupes situationnels (au lieu de 5 pour le Niveau 1, la matière
   étant plus réduite — 16 dialogues contre 37 thèmes). Émojis
   volontairement différents de ceux du Niveau 1 (même si certains
   couvrent des idées proches, ex. "Vie Quotidienne" 🏡 vs "Vie
   Quotidienne & Santé" 🏠) pour qu'un coup d'œil suffise à distinguer
   les 2 niveaux. */
const LEVEL2_CATEGORIES = [
  { id: 'contact',    emoji: '👋', labelEs: 'Primer Contacto',    labelFr: 'Premiers Contacts',
    themeIds: ['salut2','pres2','gustos2'] },
  { id: 'deplacer2',  emoji: '🧭', labelEs: 'Moverse y el Clima', labelFr: 'Se Déplacer & Climat',
    themeIds: ['chemin2','transp2','meteo2','heuretemp2'] },
  { id: 'sorties2',   emoji: '🍽️', labelEs: 'Salir y Consumir',   labelFr: 'Sorties & Consommation',
    themeIds: ['bar2','resto2','compras2','hotel2','fiesta2'] },
  { id: 'quotidien2', emoji: '🏠', labelEs: 'Vida Diaria y Salud', labelFr: 'Vie Quotidienne & Santé',
    themeIds: ['logement2','viequot2','farmacia2','medico2'] }
];

/* Retourne le prochain thème à proposer dans la carte "Continuer" :
   priorité à un thème déjà entamé (state 'progress'), sinon le 1er
   thème jamais ouvert (state 'new'), dans l'ordre de data-fr.js/
   data-es.js. Retourne null si le niveau entier est à 3 étoiles. */
function _findNextThemeToContinue(level) {
  const themes = ALL_THEMES.filter(t => t.level === level);
  return themes.find(t => getModuleState(t.id) === 'progress')
      || themes.find(t => getModuleState(t.id) === 'new')
      || null;
}

/* Construit le HTML de la carte "▶ Continuer" affichée en tête de
   grille — raccourci direct vers openTheme() sans avoir à chercher
   la bonne carte dans la grille catégorisée. Affiche un message de
   félicitations à la place si le niveau est déjà entièrement à
   3 étoiles (aucune action au clic dans ce cas). */
function _buildContinueCard(level) {
  const next = _findNextThemeToContinue(level);
  if (!next) {
    return '<div class="continue-card continue-card--done">'
      + '<span class="continue-card-emoji" aria-hidden="true">🏆</span>'
      + '<div class="continue-card-text">'
      + '<div class="continue-card-title">' + L('¡Todo completado!', 'Tout est terminé !') + '</div>'
      + '<div class="continue-card-sub">' + L('Puedes repasar cualquier tema cuando quieras', "Tu peux réviser n'importe quel thème quand tu veux") + '</div>'
      + '</div></div>';
  }
  const { main } = _themeTitle(next);
  const isNew = getModuleState(next.id) === 'new';
  const verb = isNew ? L('Empezar', 'Commencer') : L('Seguir con', 'Continuer');
  return '<button class="continue-card" onclick="openTheme(\'' + next.id + '\')">'
    + '<span class="continue-card-emoji" aria-hidden="true">' + next.emoji + '</span>'
    + '<div class="continue-card-text">'
    + '<div class="continue-card-label">▶ ' + verb + '</div>'
    + '<div class="continue-card-title">' + main + '</div>'
    + '</div>'
    + '<span class="continue-card-arrow" aria-hidden="true">→</span>'
    + '</button>';
}

/* Construit le HTML de la grille de thèmes d'un niveau — Niveau 1
   (37 thèmes, THEME_CATEGORIES) et Niveau 2 (16 dialogues,
   LEVEL2_CATEGORIES, depuis le 27/08/2026) regroupés de la même
   façon en catégories repliables. */
function _buildThemeGridHTML(level) {
  const themesForLevel = ALL_THEMES.filter(t => t.level === level);
  const categories = (level === 1) ? THEME_CATEGORIES : LEVEL2_CATEGORIES;
  ...
  let currentIdx = categories.findIndex(cat =>
    cat.themeIds.some(id => {
      const t = themesForLevel.find(x => x.id === id);
      return t && getModuleState(t.id) !== 'complete';
    })
  );
  if (currentIdx === -1) currentIdx = 0;

  return categories.map((cat, idx) => {
    const catThemes = cat.themeIds
      .map(id => themesForLevel.find(t => t.id === id))
      .filter(Boolean);
    ...
    return '<details class="theme-category"' + (isOpen ? ' open' : '') + '>'
      ...
      + '<span class="theme-category-emoji" aria-hidden="true">' + cat.emoji + '</span>'
      + '<span class="theme-category-label">' + L(cat.labelEs, cat.labelFr) + '</span>'
      + '<span class="theme-category-count">' + catDone + ' / ' + catThemes.length + '</span>'
      + '</summary>'
      + '<div class="themes-grid theme-subgrid">'
      + catThemes.map(t => _buildThemeCard(t)).join('')
      + '</div>'
      + '</details>';
  }).join('');
}
...
function renderSections(activeLevel) {
  ...
  /* ── Carte "Continuer" (ajoutée le 26/08/2026, suite) ── */
  const continueWrap1 = document.getElementById('continueWrap1');
  const continueWrap2 = document.getElementById('continueWrap2');
  if (continueWrap1) continueWrap1.innerHTML = _buildContinueCard(1);
  if (continueWrap2) continueWrap2.innerHTML = _buildContinueCard(2);

  /* ── Grilles de thèmes (Niveau 1 catégorisé, Niveau 2 simple) ── */
  const grid1 = document.getElementById('grid1');
  const grid2 = document.getElementById('grid2');
  if (grid1) grid1.innerHTML = _buildThemeGridHTML(1);
  if (grid2) grid2.innerHTML = _buildThemeGridHTML(2);
}
...
/* _autosizeFlashCard() — Corrige un bug de recouvrement des boutons de
   navigation (← Précédent / Suivant →) sur les cartes à conjugaison
   (module Verbes) ou à texte long.
   Cause : .fc-front et .fc-back sont en position:absolute (nécessaire
   pour l'effet de retournement 3D), donc leur hauteur réelle ne
   contribue jamais à celle de .fc, qui reste bloquée à min-height:240px.
   Un contenu plus grand (ex : tableau de conjugaison de 6 lignes) déborde
   alors visuellement de .fc — et comme .fc est un élément positionné, cet
   excédent est peint AU-DESSUS du bloc .fc-nav qui suit dans le flux
   normal (peu importe son ordre dans le DOM), rendant les boutons
   invisibles ET incliquables sous la zone de débordement.
   Corrigé le 26/08/2026 (demande utilisateur, signalement maman du
   09/08/2026 : "module des verbes en espagnol, on ne voit que Ser, on ne
   peut pas cliquer" — Ser est la 1ère carte du thème et a toujours eu une
   conjugaison, donc le bug touchait déjà la toute première carte).
   Solution : on mesure après rendu la hauteur réelle du contenu des deux
   faces (scrollHeight) et on applique la plus grande valeur à .fc via une
   hauteur explicite, pour que son propre encombrement dans le flux
   corresponde à ce qui est réellement affiché — même technique que
   _resizeOpenAccordions() (§15b) pour les accordéons du Guide. */
function _autosizeFlashCard() {
...
/* Recalcule aussi au redimensionnement (rotation d'écran, clavier virtuel…) :
   un texte qui se réenroule sur plus ou moins de lignes change scrollHeight. */
window.addEventListener('resize', _autosizeFlashCard);
...
/* _isBrandNewUser() — Ajouté le 11/07/2026 (demande utilisateur) : détecte
   ...
   quel que soit celui éventuellement déjà choisi.
     • 'pe_es_fr_done_v1' / 'pe_fr_es_done_v1' → progression (cf. §4, L.534/555)
...
/* Flag "l'apprenant est déjà allé au moins une fois sur l'écran Modules"
   ...
   par _markModulesVisited(), appelée depuis renderSections() (§7, L.2164)
